	...
	public static function ajax() {
		...
		while ( $the_query->have_posts() ) {
			...
			if ( empty($item_date_counts[$post_date]) || $item_date_counts[$post_date] < $limit ) {
				...
				$item = array ('title' => $title, 'color' => $color, 'start' => date('Y-m-d\TH:i:s', $post_timestamp), 'end' => date('Y-m-d\TH:i:s', $post_timestamp), 'url' => get_permalink($post->ID), 'post_id' => $post->ID, 'nonce' => wp_create_nonce('wpfc_item_nonce_' . $post->ID) );
